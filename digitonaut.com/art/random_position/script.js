...
function init() {
 for (var i = 0; i<ct; i++) {
     ...
     TweenMax.to(cube.scale, tp, {x:setSizeCube, y:setSizeCube, z:setSizeCube, yoyo:true, repeat:-1, ease:easing, delay:0.3+0.008*i});
     TweenMax.to(cube.position, tp, {x:0,y:0, z:0, delay:0.3+0.01*i, yoyo:true, repeat:-1, ease:easing});
...
function newPlace(obj) {
  ...
  TweenMax.to(obj.rotation, tp*2, {z:randomRotation, ease:easing, yoyo:true, repeat:-1});
...
function hola(num) {
   TweenMax.to(camera.position, 1, {z:num, ease:easing});
 }